...
local folderGroup = CreateGroupFrame(TeleportScroll, Color3.fromRGB(33, 33, 33))
folderGroup.Size = UDim2.new(1, -10, 0, 215)

local folderListLayout = Instance.new("UIListLayout")
folderListLayout.Padding = UDim.new(0, 5)
folderListLayout.SortOrder = Enum.SortOrder.LayoutOrder
folderListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
folderListLayout.VerticalAlignment = Enum.VerticalAlignment.Center
folderListLayout.Parent = folderGroup

local boxRow = Instance.new("Frame")
boxRow.Size = UDim2.new(1, -10, 0, 35)
boxRow.BackgroundTransparency = 1
boxRow.LayoutOrder = 1
boxRow.Parent = folderGroup
...
local t1 = AddToggle(folderGroup, "TP to all parts", "TPAllParts", function(state)
    if state then
        while Settings.TPAllParts do
            local folder = Workspace:FindFirstChild(Settings.TargetFolder)
            if folder then
                for _, v in ipairs(folder:GetChildren()) do
                    if not Settings.TPAllParts then break end
...
t1.LayoutOrder = 2
t1.Size = UDim2.new(1, -10, 0, 35)

local t2 = AddToggle(folderGroup, "TP to all models", "TPAllModels", function(state)
    if state then
...
t2.LayoutOrder = 3
t2.Size = UDim2.new(1, -10, 0, 35)

local t3 = AddToggle(folderGroup, "Bring all parts", "BringAllParts", function(state)
    if state then
...
t3.LayoutOrder = 4
t3.Size = UDim2.new(1, -10, 0, 35)

local t4 = AddToggle(folderGroup, "Bring all models", "BringAllModels", function(state)
    if state then
...
AddButton(ToolsScroll, "HoldDuration 0", function()
    for _, v pairs(Workspace:GetDescendants()) do if v:IsA("ProximityPrompt") then v.HoldDuration = 0 end end
end)
