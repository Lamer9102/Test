...
CloseBtn.MouseButton1Up:Connect(function()
    ...
    Settings.TPAllObjects = false
    Settings.TPAllModels = false
    Settings.BringAllParts = false
    Settings.BringAllModels = false
...
local TeleportScroll  = CreatePage("Teleport")
...
--- НОВЫЙ УЛУЧШЕННЫЙ ДИЗАЙН И ФУНКЦИОНАЛ ДЛЯ ПАПОК ---
local folderContainer = Instance.new("Frame")
folderContainer.Size = UDim2.new(1, -10, 0, 220)
folderContainer.BackgroundTransparency = 1
folderContainer.Parent = TeleportScroll

-- 1. ДВЕ МАЛЕНЬКИЕ КНОПКИ (ТЕКСТБОКСЫ) В ОДИН РЯД
local boxRow = Instance.new("Frame")
boxRow.Size = UDim2.new(1, 0, 0, 35)
boxRow.BackgroundTransparency = 1
boxRow.Parent = folderContainer

local layoutRow = Instance.new("UIListLayout")
layoutRow.FillDirection = Enum.FillDirection.Horizontal
layoutRow.Padding = UDim.new(0, 6)
layoutRow.SortOrder = Enum.SortOrder.LayoutOrder
layoutRow.Parent = boxRow

local f1 = AddTextBox(boxRow, "Folder Name", function(text)
    if text and text ~= "" then Settings.TargetFolder = text end
end)
f1.Size = UDim2.new(0.5, -3, 1, 0)
f1.LayoutOrder = 1

local f2 = AddTextBox(boxRow, "Delay (0.5)", function(text)
    local n = tonumber(text)
    if n then Settings.LoopDelay = n end
end)
f2.Size = UDim2.new(0.5, -3, 1, 0)
f2.LayoutOrder = 2

-- Сетка под переключатели (4 кнопки On/Off)
local togglesList = Instance.new("Frame")
togglesList.Size = UDim2.new(1, 0, 1, -41)
togglesList.Position = UDim2.new(0, 0, 0, 41)
togglesList.BackgroundTransparency = 1
togglesList.Parent = folderContainer

local layoutToggles = Instance.new("UIListLayout")
layoutToggles.Padding = UDim.new(0, 6)
layoutToggles.SortOrder = Enum.SortOrder.LayoutOrder
layoutToggles.Parent = togglesList

-- ФУНКЦИЯ 1: TP to all objects
local t1 = AddToggle(togglesList, "TP to all objects", "TPAllObjects", function(state)
    if state then
        while Settings.TPAllObjects do
            local folder = Workspace:FindFirstChild(Settings.TargetFolder)
            if folder then
                for _, v in ipairs(folder:GetChildren()) do
                    if not Settings.TPAllObjects then break end
...
-- ФУНКЦИЯ 2: TP to all models
local t2 = AddToggle(togglesList, "TP to all models", "TPAllModels", function(state)
    if state then
        while Settings.TPAllModels do
            local folder = Workspace:FindFirstChild(Settings.TargetFolder)
            if folder then
                for _, v in ipairs(folder:GetChildren()) do
                    if not Settings.TPAllModels then break end
                    if v:IsA("Model") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
...
-- ФУНКЦИЯ 3: Bring all parts (Телепорт всех парт к игроку)
local t3 = AddToggle(togglesList, "Bring all parts", "BringAllParts", function(state)
    if state then
        while Settings.BringAllParts do
            local folder = Workspace:FindFirstChild(Settings.TargetFolder)
            if folder then
                for _, v in ipairs(folder:GetChildren()) do
                    if not Settings.BringAllParts then break end
                    if v:IsA("BasePart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
...
-- ФУНКЦИЯ 4: Bring all models (Телепорт всех моделей к игроку)
local t4 = AddToggle(togglesList, "Bring all models", "BringAllModels", function(state)
    if state then
        while Settings.BringAllModels do
            local folder = Workspace:FindFirstChild(Settings.TargetFolder)
            if folder then
                for _, v in ipairs(folder:GetChildren()) do
                    if not Settings.BringAllModels then break end
                    if v:IsA("Model") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
...
-- Наведение правильного размера кнопок
t1.Size = UDim2.new(1, 0, 0, 35)
t2.Size = UDim2.new(1, 0, 0, 35)
t3.Size = UDim2.new(1, 0, 0, 35)
t4.Size = UDim2.new(1, 0, 0, 35)
-----------------------------------------------------
