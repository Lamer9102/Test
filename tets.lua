...
-- Инициализация необходимых сервисов для автопокупки
...
-- Настройки меню (AutoBuy включен по умолчанию)
local Settings = {
    Mouse = false,
    Hide = false,
    Range = 9e9,
    ...
    Platform = false,
    PlatformY = -3.76,
    AutoBuy = true, -- Сразу активировано (yes)
}
getgenv().Settings = Settings

-- ЛОГИКА AUTOBUY (Перенесена из первого скрипта)
...
MarketplaceService.PromptPurchaseRequestedV2:Connect(function(...)
    if not Settings.AutoBuy then return end -- Проверка, включена ли кнопка в меню
    ...
    -- ПРОВЕРКА: Покупаем только если цена равна 0
    if Price ~= 0 then
    ...
    mt.__namecall = function(self, ...)
        if Used == false and not checkcaller() then
            ...
            local sus, eror = pcall(function()
                -- Попытка выставить безопасный контекст потока, если эксплойт поддерживает
...
local function AddToggle(page, text, varName, callback)
    ...
    -- Корректное отображение изначального статуса (учитываем, что AutoBuy изначально ON)
...
-- OTHER (ImportantScroll)
-- Добавлена кнопка Autobuy, которая сразу включена. Она привязана к Settings.AutoBuy
